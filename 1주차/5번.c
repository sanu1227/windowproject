...
void reserveSeat() {
    ...
    if (count < 1 || count > 4) {
        printf("1~4개 좌석만 예약 가능\n");
        return;
    }

    // 시작 좌석 입력
    printf("시작 좌석 선택 (행 1~10 열 1~10): ");
    scanf("%d %d", &row, &col);
    ...
    // count개 예약 시 열 범위를 넘는 경우
    if (col + count - 1 >= MAX_SEATS) {
        printf("예약 좌석 개수가 한 열 범위 10을 넘음\n");
        return;
    }

    // 처음 좌석부터 검사
    if (theaters[tIndex].shows[sIndex].seats[row][col].reservedNum != 0) {
        printf("처음 입력한 좌석이 이미 예약됨\n");
        return;
    }

    // 연속 좌석들 중 일부가 이미 예약된 경우 검사
    for (j = 0; j < count; j++) {
        if (theaters[tIndex].shows[sIndex].seats[row][col + j].reservedNum != 0) {
            printf("연속 좌석 중 일부가 이미 예약됨\n");
            return;
        }
    }
    ...
    // 예약 처리
    for (j = 0; j < count; j++) {
        theaters[tIndex].shows[sIndex].seats[row][col + j].reservedNum = currentReservation;
    }
...
void cancelReservation() {
    ...
    if (found)
        printf("예약번호 %02d 에 해당하는 좌석이 모두 취소됨\n", resNum);
    else
        printf("예약번호 없음\n");
}
